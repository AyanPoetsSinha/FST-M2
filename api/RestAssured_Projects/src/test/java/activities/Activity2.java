...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "ayans") // Pass request body from file
        ...
        response.then().body("username", equalTo("ayans"));
        response.then().body("firstName", equalTo("Ayan"));
        response.then().body("lastName", equalTo("Sinha"));
        response.then().body("email", equalTo("ayansinha@mail.com"));
    ...
    public void deleteUser() throws IOException {
        Response response = 
            given().contentType(ContentType.JSON) // Set headers
            .pathParam("username", "ayans") // Add path parameter
            .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("ayans"));
    }
}